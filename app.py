...
ASD_INFO = {
    ...
    "affiliazione": "FIGC / LND - C.R. Liguria",
    "indirizzo": "Via dello Stadio, 1",
    "comune": "Ameglia",
    "provincia": "SP"
}

st.set_page_config(page_title="Gestionale ASD Calcio a 11 (L. 398/98)", layout="wide", page_icon="⚽")
st.title("⚽ Gestionale A.S.D. Calcio a 11 - Regime Legge 398/98")
...
if menu == "Dashboard & Alert Scadenze":
    ...
    df_cert_alert = pd.read_sql_query(f"""
    ...
    # KPI Generali
    num_tesserati = pd.read_sql_query("SELECT COUNT(*) as tot FROM tesserati_calcio WHERE stato='ATTIVO'", conn)['tot'].iloc[0]
    tot_sponsor = pd.read_sql_query("SELECT COALESCE(SUM(imponibile), 0.0) as tot FROM fatture_sponsor_398", conn)['tot'].iloc[0]
    iva_50_versare = pd.read_sql_query("SELECT COALESCE(SUM(iva_da_versare_50), 0.0) as tot FROM fatture_sponsor_398", conn)['tot'].iloc[0]
    tot_quote = pd.read_sql_query("SELECT COALESCE(SUM(importo), 0.0) as tot FROM ricevute_istituzionali", conn)['tot'].iloc[0]
    ...
    if not df_cert_alert.empty:
        ...
        st.success("✅ Tutti i calciatori risultano con certificato medico agonistico in corso di validità.")
...
# ---------------------------------------------------------
    ...
    with st.expander("➕ Inserisci Nuovo Calciatore / Tesserato"):
        with st.form("nuovo_calciatore", clear_on_submit=True):
            ...
            categoria = c4.selectbox("Categoria Squadra", ["PRIMA_SQUADRA", "JUNIORES", "ALLIEVI", "GIOVANISSIMI", "SCUOLA_CALCIO", "STAFF"])
            ruolo = c5.selectbox("Ruolo", ["Portiere", "Difensore", "Centrocampista", "Attaccante", "Allenatore", "Dirigente"])
            quota = c6.number_input("Quota Stagionale Concordata (€)", min_value=0.0, step=50.0)
            ...
            if st.form_submit_button("Salva Calciatore e Certificato"):
                if nome:
                    cursor = conn.cursor()
                    cursor.execute("INSERT INTO anagrafiche (tipo, denominazione, codice_fiscale) VALUES ('SOCIO_CALCIATORE', ?, ?)", (nome, cf))
                    ...
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?)
                    """, (anag_id, matricola, categoria, ruolo, str(datetime.date.today()), quota))
...
# ---------------------------------------------------------
    ...
    st.subheader("📜 Quota Iscrizione e Frequenza Calciatori (Decommercializzata)")

    df_calc = pd.read_sql_query("SELECT a.id, a.denominazione FROM tesserati_calcio t JOIN anagrafiche a ON t.anagrafica_id = a.id", conn)
    ...
    if not df_calc.empty:
        with st.form("nuova_ricevuta", clear_on_submit=True):
            ...
            calc_sel = col3.selectbox("Calciatore / Tesserato", list(calc_dict.keys()))

            c4, c5, c6 = st.columns(3)
            causale = c4.text_input("Causale", value="Quota iscrizione e frequenza Campionato Calcio a 11 2026/2027")
            importo = c5.number_input("Importo Incassato (€)", min_value=0.0, step=25.0)
            pagamento = c6.selectbox("Modalità Pagamento", ["BONIFICO", "POS", "PAYPAL", "CONTANTI"])

            bollo = 2.00 if importo > 77.47 else 0.00
            st.caption(f"Trattamento fiscale: **Fuori Campo IVA ex Art. 4 D.P.R. 633/72** | Marca da bollo virtuale/cartacea: **€ {bollo:.2f}**")
            ...
            if st.form_submit_button("Emetti Ricevuta e Registra in Prima Nota"):
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (num_ric, str(data_ric), calc_dict[calc_sel], causale, importo, pagamento, bollo))
                
                # Scrittura in Prima Nota Istituzionale (Cassa/Banca vs Ricavi Istituzionali 50.01.002)
                cursor.execute("""
                ...
                cursor.execute("SELECT id FROM piano_dei_conti WHERE codice = ?", (cod_cassa_banca,))
                acc_fin = cursor.fetchone()[0]
                cursor.execute("SELECT id FROM piano_dei_conti WHERE codice = '50.01.002'")
                acc_ric = cursor.fetchone()[0]

                cursor.execute("INSERT INTO righe_prima_nota (movimento_id, sottoconto_id, descrizione, dare, avere) VALUES (?, ?, 'Incasso Quota', ?, 0.0)", (mov_id, acc_fin, importo))
                cursor.execute("INSERT INTO righe_prima_nota (movimento_id, sottoconto_id, descrizione, dare, avere) VALUES (?, ?, 'Ricavo Quota Istituzionale', 0.0, ?)", (mov_id, acc_ric, importo))

                conn.commit()
                st.success(f"Ricevuta N. {num_ric} emessa con successo e registrata in contabilità!")
                st.rerun()
...
# ---------------------------------------------------------
    ...
    with st.expander("➕ Aggiungi Nuova Azienda Sponsor"):
        with st.form("nuovo_sponsor", clear_on_submit=True):
            ...
            if st.form_submit_button("Salva Sponsor"):
                cursor = conn.cursor()
                cursor.execute("INSERT INTO anagrafiche (tipo, denominazione, partita_iva, codice_fiscale, codice_destinatario) VALUES ('SPONSOR', ?, ?, ?, ?)",
                               (s_nome, s_piva, s_cf, s_sdi))
                conn.commit()
                st.success(f"Sponsor {s_nome} registrato!")
                st.rerun()

    df_sponsor = pd.read_sql_query("SELECT id, denominazione FROM anagrafiche WHERE tipo='SPONSOR'", conn)
    ...
    if not df_sponsor.empty:
        ...
        with st.form("nuova_fattura_sponsor", clear_on_submit=True):
            ...
            imponibile = st.number_input("Imponibile (€)", min_value=0.0, step=100.0)
            aliquota = 22.0
            ...
            st.info(f"📊 **Prospetto 398/98:** Totale da Incassare: **€ {totale_fattura:.2f}** | IVA a Debito Forfettizzata (50%): **€ {iva_da_versare_50:.2f}** | Stima IRES (3%): **€ {stima_ires_3:.2f}**")
            ...
            if st.form_submit_button("Registra Fattura 398/98 in Archivio e Registro"):
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (num_fat, str(data_fat), spon_dict[spon_sel], oggetto, imponibile, aliquota, iva_totale, totale_fattura, iva_da_versare_50, stima_ires_3))
                fat_id = cursor.lastrowid

                # Scrittura in Prima Nota Commerciale
                cursor.execute("""
                    INSERT INTO movimenti_prima_nota (data_registrazione, numero_documento, causale, tipo_attivita)
                ...
                cursor.execute("SELECT id FROM piano_dei_conti WHERE codice = '10.02.002'")
                acc_cred = cursor.fetchone()[0]
                cursor.execute("SELECT id FROM piano_dei_conti WHERE codice = '50.02.001'")
                acc_ric = cursor.fetchone()[0]
                cursor.execute("SELECT id FROM piano_dei_conti WHERE codice = '20.02.001'")
                acc_iva = cursor.fetchone()[0]

                cursor.execute("INSERT INTO righe_prima_nota (movimento_id, sottoconto_id, descrizione, dare, avere) VALUES (?, ?, 'Credito v/Sponsor', ?, 0.0)", (mov_id, acc_cred, totale_fattura))
                cursor.execute("INSERT INTO righe_prima_nota (movimento_id, sottoconto_id, descrizione, dare, avere) VALUES (?, ?, 'Ricavo Sponsor 398', 0.0, ?)", (mov_id, acc_ric, imponibile))
                cursor.execute("INSERT INTO righe_prima_nota (movimento_id, sottoconto_id, descrizione, dare, avere) VALUES (?, ?, 'IVA 398/98 50%', 0.0, ?)", (mov_id, acc_iva, iva_da_versare_50))

                conn.commit()
                st.success("Fattura Sponsor registrata con successo per il commercialista e nel Registro 398/98!")
                st.rerun()
    ...
    with tab_comp:
        ...
        with st.form("nuovo_compenso", clear_on_submit=True):
            if not df_staff.empty:
                staff_dict = dict(zip(df_staff['denominazione'] + " (" + df_staff['mansione'] + ")", df_staff['id']))
                staff_sel = st.selectbox("Seleziona Collaboratore Sportivo", list(staff_dict.keys()))
            else:
                st.warning("Registra prima il collaboratore nella sezione Anagrafiche Generali.")
                staff_sel = None

            c1, c2, c3 = st.columns(3)
            data_erog = c1.date_input("Data Erogazione")
            causale_c = c2.text_input("Causale", value="Compenso attività sportiva allenatore Mese")
            importo_lordo = c3.number_input("Importo Lordo (€)", min_value=0.0, step=50.0)

            if st.form_submit_button("Eroga Compenso e Calcola Franchigie"):
                if staff_sel:
                    # Calcolo franchigie €5.000 (INPS) e €15.000 (IRPEF)
                    cursor = conn.cursor()
                    coll_id = staff_dict[staff_sel]
                    prog_inps = pd.read_sql_query(f"SELECT COALESCE(SUM(importo_lordo), 0.0) as tot FROM compensi_sportivi WHERE collaboratore_id={coll_id}", conn)['tot'].iloc[0] + importo_lordo
                    
                    rit_inps = (prog_inps - 5000.0) * 0.25 * 0.50 if prog_inps > 5000.0 else 0.0
                    ...
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (coll_id, str(data_erog), causale_c, importo_lordo, prog_inps, prog_inps, rit_inps, rit_irpef, importo_netto))

                    conn.commit()
                    st.success(f"Compenso registrato! Progressivo Annuo: € {prog_inps:.2f} | Netto a Pagar: € {importo_netto:.2f}")
                    st.rerun()

    with tab_rimb:
        df_anag_all = pd.read_sql_query("SELECT id, denominazione FROM anagrafiche", conn)
        with st.form("nuovo_rimborso", clear_on_submit=True):
            ...
            st.caption(f"Calcolo Totale Rimborso Spesa Trasferta Fuori Comune: **€ {tot_rimborso:.2f}**")
            ...
            if st.form_submit_button("Registra Distinta Rimborso Trasferta"):
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (a_dict[atleta_sel], str(data_partita), incontro, luogo, km, tariffa_aci, spese_doc, tot_rimborso))
                conn.commit()
...
# ---------------------------------------------------------
    ...
    uploaded_file = c_f2.file_uploader("Carica File Estratto Conto (.csv / .xlsx)", type=["csv", "xlsx", "xls"])
...
# ---------------------------------------------------------
    ...
    with tab_rendiconto:
        st.markdown("### 🏛️ Rendiconto Economico-Finanziario Annuale (Assemblea Soci / RAS)")
