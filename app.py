...
# Inizializzazione Database (crea le tabelle ed esegue l'auto-popolamento se vuoto)
init_db()

# Titolo dinamico da Secrets (con valore generico di fallback per GitHub)
try:
...
# ---------------------------------------------------------
    ...
    with st.expander("➕ Inserisci Singolo Calciatore / Tesserato"):
        with st.form("nuovo_calciatore", clear_on_submit=True):
            ...
            quota = c6.number_input("Quota Stagionale / Frequenza (€)", min_value=0.0, step=50.0)
    ...
    # Expander 2: Importazione Massiva da Excel
    with st.expander("📥 Importazione Una Tantum Calciatori da File Excel (.xlsx)"):
        uploaded_excel = st.file_uploader("Seleziona il file Excel dei tesserati da caricare", type=["xlsx", "xls"])
            ...
            try:
                ...
                if st.button("🚀 Conferma Importazione Dati nel Database"):
                    ...
                    for _, row in df_import.iterrows():
                        ...
                        matricola = str(row.get('Matricola_FIGC', '')).strip() if pd.notna(row.get('Matricola_FIGC')) else ""
                        categoria = str(row.get('Categoria', 'PRIMA_SQUADRA')).strip() if pd.notna(row.get('Categoria')) else "PRIMA_SQUADRA"
                        ruolo = str(row.get('Ruolo', 'Calciatore')).strip() if pd.notna(row.get('Ruolo')) else "Calciatore"
                        quota_assoc = float(row.get('Quota_Associativa', 0.0)) if pd.notna(row.get('Quota_Associativa')) else 0.0
                        quota_stag = float(row.get('Quota_Stagionale', 0.0)) if pd.notna(row.get('Quota_Stagionale')) else 0.0
    ...
    if not df_soci.empty:
        ...
        with st.form("nuova_ricevuta", clear_on_submit=True):
            ...
            tipo_causale = c4.selectbox(
                "Tipologia Quota", 
                ["Quota Associativa Annua", "Quota Iscrizione e Frequenza Campionato", "Contributo Istituzionale Straordinario"]
            )
            causale_custom = c5.text_input("Dettaglio / Anno Sociale", value="Anno 2026")
            importo = c6.number_input("Importo Incassato (€)", min_value=0.0, step=10.0)
            ...
            st.caption(f"Trattamento fiscale: **Decommercializzata / Fuori Campo IVA ex Art. 4 D.P.R. 633/72** | Marca da bollo (se > € 77,47): **€ {bollo:.2f}**")
            ...
            if st.form_submit_button("Emetti Ricevuta e Registra in Prima Nota"):
                if importo > 0:
                    ...
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, (num_ric, str(data_ric), soci_dict[socio_sel], causale_completa, importo, pagamento, bollo))
                    ...
                    cursor.execute("SELECT id FROM piano_dei_conti WHERE codice = ?", (cod_ricavo,))
                    ...
                    cursor.execute("INSERT INTO righe_prima_nota (movimento_id, sottoconto_id, descrizione, dare, avere) VALUES (?, ?, 'Ricavo Istituzionale', 0.0, ?)", (mov_id, acc_ric, importo))

                    conn.commit()
                    st.success(f"Ricevuta N. {num_ric} emessa per {socio_sel} e contabilizzata!")
                    st.rerun()
                else:
                    st.error("Inserisci un importo maggiore di zero.")
    else:
        st.info("Nessun socio presente in anagrafica. Aggiungi il primo socio dal riquadro sopra per emettere le ricevute.")
...
# ---------------------------------------------------------
    ...
    if not df_sponsor.empty:
        ...
        with st.form("nuova_fattura_sponsor", clear_on_submit=True):
            ...
            st.info(
                f"📊 **Prospetto 398/98:** Totale da Incassare: **€ {totale_fattura:.2f}** | "
                f"IVA a Debito Forfettizzata (50%): **€ {iva_da_versare_50:.2f}** | Stima IRES (3%): **€ {stima_ires_3:.2f}**"
            )
